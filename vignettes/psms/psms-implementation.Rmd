...
The `adaptive` argument is now the main switch that separates a fully literal retained-mode evaluation from a more pragmatic backscatter evaluation. The important point is that `adaptive = TRUE` does not redefine the PSMS mathematics. It changes how aggressively the implementation is allowed to stop once the retained modal tail has already become numerically inactive.
...
In the second call, `n_integration` is omitted on purpose. For the full fluid-filled or gas-filled PSMS solve, `adaptive = TRUE` chooses the quadrature order internally from the retained modal difficulty and ignores a user-supplied `n_integration`.

## Benchmark comparisons

The PSMS implementation is easiest to interpret when the main numerical switches are compared directly against the stored benchmark spectra. The table below restores the broader comparison across `precision`, `simplify_Amn`, and `adaptive`. The runs use `12, 18, 38, 70, 120, 200, 250, 300, 333, 400 kHz` against the bundled `benchmark_ts` data.
...
2. In Jech et al. (2015, Table III), `NB` is defined as "no benchmark." The gas-filled prolate-spheroid frequency comparison is marked `NB`, and the corresponding discussion states that no benchmark `TS` values were calculated for that target. The gas-filled rows below therefore report `NB` in the delta columns rather than treating that curve as a benchmark target.
3. The liquid-filled benchmark has values at `12, 18, 38, 70, 120, 200, 250, 300, 400 kHz`, but not at `333 kHz`.

The reported deltas are therefore computed only where validated benchmark values exist.
...
1. For `fixed_rigid` and `pressure_release`, both precisions remain benchmark-close over the frequencies for which benchmark values are available, and the adaptive early-stop logic does not materially change those benchmark deltas on this short grid.
...
5. The gas-filled prolate rows are retained here because they are still part of the implementation space, but they are not interpreted as benchmark deltas because the published comparison explicitly labels that case `NB` for no benchmark.
6. The current adaptive liquid-filled path is useful in quad precision, but it is not universally beneficial. In particular, the `precision = "double"`, `simplify_Amn = FALSE`, `adaptive = TRUE` combination drifts much farther from benchmark on this grid than the literal double-precision run.
7. The largest liquid-filled deviations still occur in deep null neighborhoods, where small phase or truncation differences can produce visibly larger `TS` differences in dB than they would on a linear scattering-amplitude scale.
...
par(mfrow = c(2, 1), mar = c(4.3, 4.8, 2, 1), oma = c(0, 0.2, 0, 0))
plot(
  precision_df$k0b,
...
plot(
  ...
  lwd = 2,
  xlab = expression(k[0] * b),
  ylab = expression(Delta ~ TS[double - quad] ~ (dB)),
  col = "steelblue4"
)
abline(h = 0, lty = 3, col = "grey50")
```

For this full liquid-filled run, the double- and quadruple-precision curves are effectively indistinguishable through `k_0 b < 1`, differ by only about `1.4e-07 dB` at `38 kHz`, then separate to about `0.01 dB` by `70 kHz` (`k_0 b â‰ˆ 2.98`) and to about `1.02 dB` by `100 kHz` (`k_0 b â‰ˆ 4.25`). That is the practical reason the benchmark table above starts to favor quadruple precision once the retained PSMS system moves into the higher-frequency penetrable regime.
...
3. The gas-filled rows are much cheaper here because the currently benchmarked gas-filled frequency range is short (`12-40 kHz`) and does not drive the same modal growth as the liquid-filled `12-400 kHz` sweep.
4. The current adaptive liquid-filled path is faster because it combines a smaller modal-content-based quadrature rule at lower reduced frequency with earlier \(n\)- and \(m\)-tail termination inside the backscatter assembly.
5. On this machine and grid, that reduces the full liquid-filled quad run from about `59.7 s` to about `48.3 s` without materially changing benchmark agreement.
6. The same statement is not automatically true in double precision: the adaptive full liquid-filled double run is faster here, but the benchmark table above shows that it also drifts much farther from the benchmark curve.
7. In this adaptive liquid-filled quad comparison, the internally selected quadrature orders were `32, 32, 32, 32, 32, 48, 56, 64, 72, 88`; that sequence is shown in the frequency-by-frequency table above because it is an internal implementation detail, not a user-specified input.

## Cross-software implementation checks

Beyond the published benchmark curves, it is also useful to check whether the same prolate-spheroid definitions produce comparable spectra in other locally available implementations. The table below uses the shared `12, 18, 38, 70, 100 kHz` frequency set so that the cross-software check reaches more informative reduced frequencies while still remaining computationally manageable. The `acousticTS` settings are the same ones used for the main benchmark-quality run here: `precision = "quad"`, `adaptive = FALSE`, `simplify_Amn = FALSE`, `n_integration = 96`, and `phi_body = pi`.

`Prol_Spheroid` only treats penetrable prolate spheroids, so its cells are `N/A` for `fixed_rigid` and `pressure_release`. The table separates the original and vectorized `Prol_Spheroid` branches because the numerical agreement is nearly identical while the runtime is not.
...
By contrast, the shared `echoSMs::PSMSModel` comparison begins to separate at the higher two frequencies, especially for the penetrable cases: the liquid-filled difference reaches about `1.02 dB` at `100 kHz`, while the gas-filled comparisons diverge much more strongly above `38 kHz`. The gas-filled `Prol_Spheroid` timings also show how expensive the original branch can become at this five-frequency set, taking about `698.5 s` versus `26.9 s` for the vectorized branch with no meaningful change in the returned `TS`. In other words, the extended frequency set is doing exactly what it should do here: it shows that the low-frequency agreement is reassuring, but it also flags the higher-frequency penetrable PSMS pathways as the part of the implementation space that still deserves the most careful future scrutiny.
