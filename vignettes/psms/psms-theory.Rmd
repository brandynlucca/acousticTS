...
The prolate spheroidal modal series solution (PSMS) is the natural exact-separation analogue of spherical partial-wave theory for elongated bodies whose surface is better approximated by a prolate spheroid than by a sphere or cylinder [@Spence_1951; @Silbiger_1963]. The essential fact is that the Helmholtz equation is separable in prolate spheroidal coordinates, so the incident, scattered, and interior fields can all be expanded in spheroidal angular and radial wave functions [@Spence_1951; @Silbiger_1963; @Furusawa_1988; @Yeh_1967; @Flammer_1957; @Ye_1997_2].
...
the fluid-filled PSMS system.](psms-overlap-projection-schematic.png) -->
...
where $\lambda_{mn}(h)$ is the separation constant. When $h \to 0$, this reduces to the associated Legendre equation, so the spheroidal angular functions reduce smoothly to Legendre functions [@Flammer_1957; @DLMF:ch30; @DLMF:ch14].
...
The fluid-filled case is more involved because the interior field uses a different reduced frequency, $\mathbb{k}_1 = k_2 q$, and therefore a different spheroidal basis [@Yeh_1967; @Furusawa_1988; @Ye_1997_2]. The exterior and interior angular functions are not identical when $\mathbb{k}_1 \ne \mathbb{k}_1$, so the boundary conditions do not remain diagonal in $n$. The gas-filled case uses exactly the same algebraic structure as the fluid-filled case; only the interior density and sound speed differ.
...
$$
  ...
  &\sum_{n=m}^{\infty} A_{mn}S_{mn}^{(1)}(\mathbb{k}_1,\eta)R_{mn}^{(3)}(\mathbb{k}_1,\xi_1) +
    \sum_{n=m}^{\infty} S_{mn}^{(1)}(\mathbb{k}_1,\eta)R_{mn}^{(1)}(\mathbb{k}_1,\xi_1) \\ &= \sum_{\ell=m}^{\infty} B_{m\ell}S_{m\ell}^{(1)}(\mathbb{k}_1,\eta)R_{m\ell}^{(1)}(\mathbb{k}_1,\xi_1).
  \end{aligned}
$$
...
with target strength [@MacLennan_2002; @Urick_1983; @Simmonds_2005]:
