...
acousticTS:::.model_family_overview(
  ...
  core_idea = paste(
    "Solve the infinite circular elastic-cylinder interface through modal phase",
    "shifts, then apply a finite-length coherence factor. Uniform curvature can",
  ...
  reading = c(
    "[Implementation](ecms-implementation.html): supported objects, material inputs, output, and current checks",
    "[Theory](ecms-theory.html): elastic potentials, phase shifts, and finite-length closure"
  )
)
```
