...
In acousticTS, those functions are evaluated through the `profcn` backend in `src/prolate_swf.f90` [@van_buren_boisvert_prolate_swf_2026], with the corresponding `C++` wrappers in `src/psms.cpp` handling the extraction and rescaling needed by the exported `Smn()` and `Rmn()` interfaces. That split of responsibility matters because the Fortran routine computes a large family of spheroidal quantities at once, while the package-level functions usually want one specific angular or radial quantity and, often, its derivative.
