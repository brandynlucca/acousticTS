...
TMM represents a target by the linear map from incident modal coefficients to
scattered modal coefficients [@Waterman_1969; @Waterman_2009].

Build a supported canonical scatterer, solve it with `target_strength()`, and
optionally retain the modal blocks for angular or orientation post-processing.

## Why use `TMM` over `SPHMS` or `PSMS`?

On a canonical shape, TMM should reproduce the corresponding shape-specific
family when both solve the same boundary-value problem. Its added value is the
retained coefficient map, which supports bistatic scattering and orientation
averaging without repeating the boundary solve.
...
3. if the goal is only a finite-cylinder target-strength calculation, `FCMS`
   is the more direct cylinder model, and
...
::: {.note data-title="Stored-state scope"}
General-angle post-processing is available for sphere, oblate-spheroid, and
prolate-spheroid branches. Spherical `ESS` objects also support the documented
fluid-shell and elastic-shell boundaries. Cylinder storage is limited to
...
For spheres, `n_max` is the spherical-wave cutoff. For prolates, it is inherited
from the spheroidal retained system and describes the size of that solve, not a
spherical-wave truncation.
...
For a canonical shape, compare the stored monostatic result with the matching
shape-specific model before relying on post-processed angular products.
...
The angular slice shows how a stored operator distributes scattering over
receive angle at one frequency.
...
`summary_70$metrics$TS` should agree with the `TS` already stored at `70 kHz`.

### Single solved target, many post-process products

Use `tmm_products()` when several post-processing products are required from
the same solved target.
...
The stored operator can therefore supply monostatic, orientation-averaged, and
bistatic products without repeating the target solve.
...
`Sphere`, `OblateSpheroid`, and `ProlateSpheroid`. Cylinder is omitted because
its retained state does not expose a validated angular grid.
...
The retained prolate operator is checked by comparing `tmm_scattering()` with
`prolate_spheroid_fbs()` at the same frequency and angles.
...
1. the sphere has been compared with BEM/FEM across all four boundaries,
2. pressure-release oblate and prolate angular fields agree closely after the
   frame convention is matched,
3. some penetrable BEM comparisons contain large maxima near nulls and should
   be interpreted with the exact-family and mean-residual columns,
...
The sphere spectrum checks agreement with SPHMS. The prolate spectrum checks
agreement with the spheroidal solution used by PSMS.
