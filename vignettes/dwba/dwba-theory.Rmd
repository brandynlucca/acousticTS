...
The distorted wave Born approximation (DWBA) is a first-order scattering model for weakly scattering fluid-like bodies. It is most useful when the material contrasts are small enough that the total acoustic field inside the body remains close to the incident field, but the geometry is sufficiently extended that phase accumulation along the body cannot be ignored [@Morse_1968; @Stanton_1998_2]. In that regime, a purely local approximation is too crude, while a full boundary-value solution is often unnecessary or unavailable.
...
Throughout this page, medium `1` is the surrounding seawater and medium `2` is
the body interior, following the shared package conventions summarized in the
[acoustic scattering primer](../acoustic-scattering-primer/acoustic-scattering-primer.html)
and [notation guide](../notation-and-symbols/notation-and-symbols.html).
...
$e^{-i \omega t}$. Let the surrounding seawater be medium `1`, with density
$\rho_1$, sound speed $c_1$, compressibility
...
The body is therefore reduced to a coherent sum of local cross-sectional responses distributed along the centerline. Moreover, if the material-property contrast does not vary across the local section, it can be taken outside the inner integral.
...
and target strength is [@MacLennan_2002; @Urick_1983; @Simmonds_2005]:
