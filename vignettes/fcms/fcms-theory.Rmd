...
The rigid, pressure-release, and fluid-filled cylindrical boundaries used below are summarized separately in the companion [boundary conditions article](../boundary_conditions.html), but the present vignette shows how those boundary conditions enter the cylindrical coefficients and how those coefficients are folded into the finite-cylinder backscatter.
...
The first figure makes the factorization explicit: the circular cross-section carries the cylindrical modal physics through the coefficients $B_m$, while the axial extent $L$ and orientation angle $\theta$ produce the separate coherence factor $\sin(kL\cos\theta)/(kL\cos\theta)$ used later in the finite-cylinder reduction.

The second figure emphasizes the same separation from a numerical point of view. The circumference problem is where the boundary condition is handled exactly, while the finite axis enters only through a one-dimensional coherence factor. That distinction is what makes the FCMS much simpler than a fully three-dimensional endcap-resolving boundary-value problem.

The third figure then handles a different task. It is the geometry map for the reduction itself: radius $a$, length $L$, incidence angle $\theta$, transverse wavenumber $K = k\sin\theta$, and the axial phase scale $kL\cos\theta$. Keeping this figure separate from the factorization schematics makes the page easier to read because the conceptual decomposition and the coordinate bookkeeping are no longer being asked to do the same job.
