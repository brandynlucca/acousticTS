...
cylindrical-harmonic solution for a circular cross-section with an approximate
finite-axis coherence factor [@Stanton_1988; @Stanton_1989_2]. It does not
...
Medium `1` is the surrounding fluid and medium `2` is the cylinder interior.

::: {.note data-title="Shared conventions"}
Symbols and medium indexing follow [Notation and
Symbols](../notation-and-symbols/notation-and-symbols.html). The interface
conditions used below are contextualized in [Boundary
...
The modal sum enforces the boundary condition around the circumference. The
sinc factor describes interference among those local responses along a finite
segment. It does not resolve the endcaps as coupled scattering surfaces.
...
This is Bessel's equation, whose solutions are $J_m(Kr)$, $Y_m(Kr)$, and the outgoing combination $H_m^{(1)}(Kr)$ [@DLMF:ch10]. Regularity at the axis $r=0$ excludes the singular solution $Y_m$ from the incident and interior regular fields, while the outgoing radiation condition selects the Hankel function of the first kind for the scattered field [@Sommerfeld_1949].
...
The Neumann factor $\epsilon_m$ permits the cosine series to be written over
nonnegative mode numbers while retaining the contributions from positive and
negative angular orders.
...
$$
  J_m'(Ka)+B_mH_m^{(1)\prime}(Ka) = \\
    \frac{1}{g_{21}h_{21}}\frac{J_m'(K'a)}{J_m(K'a)}
...
$$
  B_m
  \left[H_m^{(1)\prime}(Ka)-\frac{1}{g_{21}h_{21}}\frac{J_m'(K'a)}{J_m(K'a)}H_m^{(1)}(Ka)\right] = \\
    \frac{1}{g_{21}h_{21}}\frac{J_m'(K'a)}{J_m(K'a)}J_m(Ka)-J_m'(Ka).
$$
...
$$
  B_m =
    \frac{\dfrac{1}{g_{21}h_{21}}\dfrac{J_m'(K'a)}{J_m(K'a)}J_m(Ka)-J_m'(Ka)}
  {H_m^{(1)\prime}(Ka)-\dfrac{1}{g_{21}h_{21}}\dfrac{J_m'(K'a)}{J_m(K'a)}H_m^{(1)}(Ka)}.
$$
...
$$
  ...
  \left[J_m'(K'a)Y_m(Ka)\right]/\left[J_m(K'a)J_m'(Ka)\right] -
    g_{21}h_{21}\left[Y_m'(Ka)/J_m'(Ka)\right]
  }{
...
Multiplying the preceding numerator and denominator by $g_{21}h_{21}$ and
separating $H_m^{(1)}=J_m+iY_m$ produces this auxiliary ratio. Thus the
$g_{21}h_{21}$ factors in $C_m$ are consistent with the reciprocal factor in
...
Once $m_{\max}$ is fixed, solve the scalar or $2\times2$ coefficient problem
for each $m=0,1,\ldots,m_{\max}$, evaluate the cross-sectional modal sum, and
apply the finite-length coherence factor. Truncation does not create a dense
cross-mode system.

Truncation leaves the boundary condition unchanged. Too few modes under-resolve
the cross-sectional field, especially at larger acoustic size.
...
The corresponding target strength is [@MacLennan_2002]:
...
FCMS therefore has exact circular-boundary physics within its cross-sectional
problem and approximate finite-length physics. Strongly varying radii,
influential endcaps, or axial material variation fall outside that
factorization.
