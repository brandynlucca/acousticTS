...
The stochastic distorted wave Born approximation (SDWBA) begins with the same weak-scattering derivation as the deterministic DWBA, then modifies the phase structure of the coherent sum to account for unresolved variability in body shape, posture, and internal structure.[^1][^2][^3]

The same weak fluid-like boundary assumptions summarized in the companion [boundary conditions article](../boundary_conditions.html) apply here before the stochastic phase extension is introduced.
...
$$
  f_{bs}(\theta) \approx \sum_{j=1}^{N} q_j(\theta),
$$
...
$$
  f_{bs}^{(m)}(\theta) = \sum_{j=1}^{N} q_j(\theta)e^{i\varphi_j^{(m)}},
$$
...
![SDWBA geometry and phase-perturbation schematic, showing deterministic segment locations, unresolved path-length offsets, and the random phase factors applied to each segment contribution.](sdwba-phase-geometry-clean.svg)
...
The schematic above is tied directly to the stochastic phase model: the labeled $q_j$ are the deterministic DWBA segment contributions, the perturbed segment markers indicate the unresolved path-length offsets $\delta \ell_j$, and the attached phase factors $e^{i\varphi_j}$ represent the resulting realization-specific correction to the deterministic phase. This is the key geometric interpretation of the SDWBA. The model is not randomizing segment amplitudes arbitrarily. It is randomizing the phase implied by a slightly perturbed effective path length.

The second schematic is meant to answer a different question. The first one explains how the random phase is inserted into the deterministic sum. The second one shows what that insertion does qualitatively as $\sigma_\varphi$ increases. Readers often understand the ensemble-average formula more quickly once they see that the self-terms remain intact while the organized interference pattern across segments is progressively suppressed.

## Why Gaussian phase noise is used

The Gaussian choice is mathematically convenient and physically reasonable when the phase perturbation is the sum of many small unresolved contributions. By the central limit principle, the cumulative phase error then tends toward a normal distribution even if the underlying geometric perturbations are not themselves Gaussian.
...
$$
  \langle \sigma_{bs}(\theta) \rangle = \mathbb{E}\!\left[\left|f_{bs}(\theta)\right|^2\right].
$$
...
$$
  \langle \sigma_{bs} \rangle
  = \sum_{j=1}^{N} |q_j|^2
  + \sum_{j \ne \ell} q_j q_\ell^* \,
    \mathbb{E}\!\left[e^{i(\varphi_j - \varphi_\ell)}\right].
$$

If the phase perturbations are independent and identically distributed with variance $\sigma_\varphi^2$, then
...
and the deterministic DWBA is recovered.
...
and only the incoherent sum of segment intensities survives:

$$
  \langle \sigma_{bs} \rangle \to \sum_{j=1}^{N}|q_j|^2.
$$
...
where $M$ is the number of realizations. The target strength is then defined from the averaged linear cross-section:

$$
  TS = 10\log_{10}\!\left(\langle \sigma_{bs}(\theta) \rangle\right).
$$
...
[^1]: Demer, D. A., and Conti, S. G. (2003). Reconciling theoretical versus empirical target strengths of krill: effects of phase variability on the distorted-wave Born approximation. *ICES Journal of Marine Science*, 60, 429-434.

[^2]: Conti, S. G., and Demer, D. A. (2006). Improved parameterization of the SDWBA for estimating krill target strength. *ICES Journal of Marine Science*, 63, 928-935.
