...
Because the geometry is spherical, the exterior and interior Helmholtz equations are separable in spherical coordinates [@achenbach_wave_1973]. For a sphere centered at the origin, the Hemholtz equation provides solutions that can be written as products of single-variable functions:
...
For a plane wave incident along polar coordinate $z$, the acoustic pressure is expressed as [@hickling_analysis_1962]:
...
It is often more informative to parameterize each partial wave by a real phase shift $\eta_m$ than by the raw complex coefficient $B_m$ [@faran_sound_1951; @rudgers_acoustic_1969]. In that representation, the elastic boundary conditions determine how far the outgoing partial wave is shifted relative to the free spherical solution.
...
where $\zeta_m(k_\ell a, \sigma)$ is the boundary-impedance phase angle induced by the elastic interior [@faran_sound_1951]. Once obtained, $B_m$ is: $$ B_m = k_1 (-1)^m (2m + 1) h_m^{(1)}(k_1 r_0) \sin \eta_m e^{-i \eta_m}. $$
...
Target strength is then reported as [@maclennan_theory_1981]:
