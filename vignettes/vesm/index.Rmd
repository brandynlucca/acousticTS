...
acousticTS:::.model_family_overview(
  ...
  supports = c(
    paste0(
      "`ESS` spherical objects with an explicit shell and inner gas core ",
      "plus viscous-layer model arguments"
    ),
  ...
  reading = c(
    paste0(
      "[Implementation](vesm-implementation.html): layered-object workflow ",
      "and benchmark comparisons"
    ),
    paste0(
      "[Theory](vesm-theory.html): full layered spherical field structure, ",
      "interface conditions, and mode-wise systems"
    )
  )
)
```
