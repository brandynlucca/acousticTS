...
VESM models a gas core, elastic shell, viscous outer layer, and surrounding
fluid [@Khodabandeloo_2021; @Feuillade_1998].
...
@Khodabandeloo_2021 over a shared frequency grid.
...
knitr::kable(
  ...
  col.names = c(
    "Comparison",
    "N frequency",
    "<i>f</i><sub>min</sub> (kHz)",
    "<i>f</i><sub>max</sub> (kHz)",
    "Max |&Delta;| <i>TS</i> (dB)",
    "Mean |&Delta;| <i>TS</i> (dB)",
    "<i>f</i> at max |&Delta;| <i>TS</i> (kHz)"
  )
)
```
...
The spectra are visually superposed across the comparison band. The residual
reaches about `0.0572 dB` near `60 kHz`, which remains small relative to the
scale of the full spectrum.
