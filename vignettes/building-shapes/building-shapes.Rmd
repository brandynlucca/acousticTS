...
The important point is that shape construction is not yet the stage where material contrasts or boundary conditions are fixed. At this point the user is deciding only how the body is described geometrically. That separation is intentional. It keeps the geometric question distinct from the physical one and makes it easier to evaluate whether a later result is being driven by morphology, by material properties, or by model assumptions.
...
For most readers, the direct generators are also the clearest interface because they make the resulting shape class explicit at the call site. `create_shape()` is still useful, but it is best treated as a thin convenience dispatcher rather than the main pedagogical path.
