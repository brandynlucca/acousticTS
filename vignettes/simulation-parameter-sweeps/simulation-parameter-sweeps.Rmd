...
This schematic is important because morphology-affecting parameters do not behave like ordinary scalar metadata. They change the working target itself before the model is run. That means the resulting simulations should be interpreted as distinct geometric states derived from one baseline object, not as repeated evaluations of a single unchanged shape.
...
The execution schematic is meant to clarify that parallelization changes scheduling, not interpretation. The same batched cells and realization-level jobs still exist. Parallel execution simply distributes them across workers. That is why debugging is often easier in serial mode first, even when the final production run is parallel.
