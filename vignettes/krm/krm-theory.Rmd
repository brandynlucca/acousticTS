...
$$
  d f_{K} \sim -i\frac{(k a\sin\theta)^{1/2}}{2\sqrt{\pi}} e^{-i2k z_U}\,du,
$$

where $du$ is projected segment length, $a$ is local radius, $k$ is the water wavenumber, $\theta$ is the angle of incidence, and $z_U$ is the upper-surface coordinate of the segment. The KRM body contribution starts from this leading Kirchhoff term and then augments it by the first transmitted traversal through the body.
...
Scattering is accumulated in the rotated $(u,v)$ frame aligned with the insonifying wave even though the body description begins in $(x,z)$. The quantities $z_U$ and $z_L$ denote the upper and lower surfaces used in the two-path body term, while $\Delta u_j$ is the projected segment length that weights each local contribution.
...
$$
  \frac{(k a_j\sin\theta)^{1/2}}{2\sqrt{\pi}}\Delta u_j.
$$
...
Each short body segment contributes two principal coherent terms. The first is a direct reflection from the dorsal surface. The second enters the body, propagates through it, reflects internally, and exits again. Retaining the standard KRM notation, the upper surface is denoted by $z_{U,j}$, while $v_j$ and $v_{L,j}$ are the rotated-coordinate quantities used in the through-body phase bookkeeping. The two contributions are:

$$
  \mathcal{R}_{wb} e^{-i2k z_{U,j}},
$$
...
$$
  ...
  e^{-i2k z_{U,j} + i2k_B(w_j-v_j-v_{L,j}) + i\psi_{B,j}},
$$
...
$$
  \frac{(k a_j\sin\theta)^{1/2}}{2\sqrt{\pi}},
$$
...
$$
  ...
  -i\frac{(k a_j\sin\theta)^{1/2}}{2\sqrt{\pi}}
  \Delta u_j
  \left[
    \mathcal{R}_{wb} e^{-i2k z_{U,j}}
    -
