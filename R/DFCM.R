...
DFCM <- function(shape=NULL, L=max(shape@rpos[,1]), a=max(shape@a), g=shape@g, h=shape@h, pc=shape@pc, numdiv=length(shape@rpos[1,]),
                 theta=shape@theta, cm=1500, pm=1.025, frequency, alpha=0.8, method="two-ray"){
  ...
  if(method == "two-ray"){
    ...
    km <- kcalc(frequency,cm)
    ka <- kcalc(frequency,ca)
    ...
    fbs <- 0.5*sqrt(pc*a_new) * R12 * exp(-1i*2*km*a_new) * Io * exp(-alpha*(2*(theta-pi/2)*pc/L)^2)
    TS <- 20*log10(abs(fbs))
    return(TS)
  }
}
