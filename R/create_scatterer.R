...
#' @param theta_bladder Angle of body relative to wavefront
...
sbf_generate <- function(x_body,
  ...
  bladder <- base::list(
    rpos = base::rbind(
      x = x_bladder[!base::is.na(x_bladder)],
...
cal_generate <- function(material = "WC",
  ...
  material_properties <- base::switch(material,
    Cu = list(
...
fls_generate <- function(shape = "arbitrary",
  ...
  # Create body slot =========================================================
  body <- list(
    rpos = t(shape_input@position_matrix),
