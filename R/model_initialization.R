...
mss_anderson_initialize <- function(object,
  ...
  ka_limit <- base::ifelse(!is.null(ka_limit),
    ka_limit,
    base::round(
      base::max(model_params$acoustics$k_sw) * body$radius
    ) + 20
  )
...
mss_goodman_stern_initialize <- function(object,
  ...
  methods::slot(
    object,
    "model"
  )$MSS_goodman_stern <- base::data.frame(
    frequency = frequency,
    sigma_bs = base::rep(
      NA,
      length(frequency)
...
#' @param alpha_B Numerical coefficient
#' (\ifelse{html}{\out{&alpha;<sub>B</sub>}}{\eqn{\alpha_{B}}}).
#' @export
...
#' @param theta_body Optional orientation input (relative to incident sound
#' wave).
#' @param theta_swimbladder Optional orientation input (relative to incident
#' sound wave).
#' @export
krm_initialize <- function(object,
  ...
  if (scatterer_type == "FLS") {
    ...
    # Body wave number =========================================================
    model_params$acoustics$k_b <- acousticTS::k(
      frequency,
    ...
    methods::slot(object, "model_parameters")$KRM <- base::list(
      parameters = model_params,
      medium = medium_params,
      body = body_params
    )
    ...
    bladder_params <- base::data.frame(
