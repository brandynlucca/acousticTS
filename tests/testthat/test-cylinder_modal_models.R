...
test_that(
  "BCMS reproduces the exact straight-cylinder kernel and bent correction",
  {
    density_sw <- 1026.8
...
test_that("BCMS infers default boundaries and guards limited options", {
  ...
  expect_equal(liquid_default@model$BCMS$TS, liquid_explicit@model$BCMS$TS,
    tolerance = 1e-12
  )
  expect_equal(gas_default@model$BCMS$TS, gas_explicit@model$BCMS$TS,
    tolerance = 1e-12
  )
...
test_that(
  "ECMS reproduces straight and bent elastic-cylinder coherence bookkeeping",
  {
    length_body <- 0.04
    radius_body <- 0.005
    radius_curvature_ratio <- 1.8
