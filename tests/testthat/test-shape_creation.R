...
test_that(
  paste0(
    "Canonical KRM profiles match the common benchmark geometries after ",
    "reversal"
  ),
  {
...
test_that(
  "gas_generate preserves canonical prolate metadata for modal-series models",
  {
    obj <- gas_generate(
...
test_that(
  "ess_generate derives the internal fluid shape from explicit Shape inputs",
  {
...
test_that(
  paste0(
    "scatterer constructors prefer explicit Shape inputs and warn on ",
    "deprecated unit args"
  ),
  {
    ...
    expect_equal(rownames(sbf_obj@body$rpos), c(
      "x_body", "w_body", "zU_body",
      "zL_body"
    ))
    expect_equal(
...
test_that(
  paste0(
    "scatterer constructors accept explicit manual coordinates without shape ",
    "strings"
  ),
  {
    obj <- fls_generate(
...
test_that(
  paste0(
    "character shape dispatch remains available but warns in scatterer ",
    "constructors"
  ),
  {
    expect_warning(
...
test_that("Oblate spheroid creation works", {
  ...
  expect_equal(max(oblate_obj@shape_parameters$radius), radius_body,
    tolerance = 1e-12
  )
  ...
  expect_equal(max(oblate_ratio@shape_parameters$radius), radius_body,
    tolerance = 1e-12
  )
...
test_that(
  paste0(
    "explicit shapes override legacy coordinate inputs in composite ",
    "constructors"
  ),
  {
    body_shape <- arbitrary(
...
test_that(
  "scatterer constructors reject mixed contrast and absolute property inputs",
  {
    expect_error(
      fls_generate(
