...
test_that(
  "TMM supports oblate spheroids through the spherical-coordinate branch",
  {
    density_sw <- 1026.8
    sound_speed_sw <- 1477.3
    ...
    oblate_sphere <- fls_generate(
      shape = oblate_spheroid(
        length_body = 0.02, radius_body = 0.01,
        n_segments = 80
      ),
      density_body = 1028.9,
    ...
    expect_equal(oblate_tmm@model$TMM$TS, sphere_sphms@model$SPHMS$TS,
      tolerance = 1e-9
    )
...
test_that(
  "Oblate TMM retained blocks support the same post-processing helpers",
  {
    density_sw <- 1026.8
    sound_speed_sw <- 1477.3
    object <- target_strength(
...
test_that(
  "TMM matches PSMS across multiple prolate spheroids and boundary cases",
  {
    density_sw <- 1026.8
    sound_speed_sw <- 1477.3
    shape_specs <- list(
    ...
    compare_prolate_case <- function(object, boundary, frequency, max_delta) {
      ...
      expect_lt(
        max(abs(tmm_obj@model$TMM$TS - psms_obj@model$PSMS$TS)),
        max_delta
      )
    }
...
test_that("TMM warns when prolate spheroids request spherical-branch options", {
  ...
  object <- fls_generate(
    shape = prolate_spheroid(
      length_body = 0.14, radius_body = 0.01,
      n_segments = 80
    ),
    g_body = 1,
    h_body = 1
...
test_that(
  "TMM can retain the frequency-specific T-matrix blocks for prolates",
  {
    density_sw <- 1026.8
    sound_speed_sw <- 1477.3
    object <- fls_generate(
...
test_that("Stored prolate TMM keeps the spheroidal modal cutoff", {
  ...
  object <- fls_generate(
    shape = prolate_spheroid(
      length_body = 0.07, radius_body = 0.01,
      n_segments = 80
    ),
    density_body = 1024,
...
test_that(
  paste0(
    "Stored prolate TMM paired-point evaluation matches scalar spheroidal ",
    "calls"
  ),
  {
    density_sw <- 1026.8
...
test_that("tmm_scattering reproduces stored monostatic results", {
  ...
  expect_equal(prolate_scat$f_scat, prolate_object@model$TMM$f_bs,
    tolerance =
      1e-10
  )
})
...
test_that(
  paste0(
    "Stored and unstored prolate TMM runs agree and stay tied to the PSMS ",
    "benchmark"
  ),
  {
    density_sw <- 1026.8
    ...
    expected_f_bs <- (-2i / wavenumber(frequency, sound_speed_sw)) *
      psms_obj@model$PSMS$f_bs
    ...
    expect_lt(
      max(Mod(tmm_stored@model$TMM$f_bs - tmm_direct@model$TMM$f_bs)),
      1e-6
    )
    expect_equal(tmm_stored@model$TMM$sigma_bs, tmm_direct@model$TMM$sigma_bs,
      tolerance = 1e-6
    )
    expect_equal(tmm_stored@model$TMM$TS, tmm_direct@model$TMM$TS,
      tolerance =
        5e-4
    )
  }
...
test_that(
  "Stored prolate TMM matches the exact general-angle pressure-release field",
  {
    density_sw <- 1026.8
    sound_speed_sw <- 1480
    theta_body <- pi / 2
...
test_that(
  "TMM orientation distributions normalize and agree with explicit averaging",
  {
    density_sw <- 1026.8
    sound_speed_sw <- 1477.3
    object <- target_strength(
    ...
    avg_from_dist <- tmm_average_orientation(object,
      distribution =
        quadrature_dist
    )
...
test_that("tmm_scattering_grid reproduces stored monostatic scattering", {
  ...
  expect_equal(grid$sigma_scat_dB[1, 1], mono$sigma_scat_dB[2],
    tolerance =
      1e-10
  )
})
...
test_that(
  paste0(
    "stored spherical TMM point scattering stays on the monostatic amplitude ",
    "scale"
  ),
  {
    density_sw <- 1026.8
...
test_that(
  "TMM bistatic summaries and product bundles are internally consistent",
  {
    density_sw <- 1026.8
    sound_speed_sw <- 1477.3
    ...
    expect_equal(summary_70$metrics$sigma_bs, monostatic_70$sigma_scat[2],
      tolerance = 1e-10
    )
    expect_equal(summary_70$metrics$TS, db(summary_70$metrics$sigma_bs),
      tolerance = 1e-12
    )
    expect_equal(
    ...
    expect_equal(products$monostatic$f_scat, tmm_scattering(object)$f_scat,
      tolerance = 1e-10
    )
    expect_equal(
...
test_that("Default cylinder TMM matches FCMS through the cylindrical backend", {
  ...
  compare_cylinder_case <- function(object, boundary, tolerance = 1e-12) {
    ...
    expect_equal(tmm_obj@model$TMM$TS, fcms_obj@model$FCMS$TS,
      tolerance =
        tolerance
    )
  }
...
test_that(
  "Cylinder TMM stores cylindrical-family retained state when requested",
  {
    old_opt <- options(acousticTS.warn_tmm_cylinder = FALSE)
    on.exit(options(old_opt), add = TRUE)
    density_sw <- 1026.8
    ...
    expect_length(t_store, 2)
    expect_true(all(vapply(t_store, is.list, logical(1))))
    expect_true(all(vapply(t_store, function(x) {
      identical(
        x$family,
        "cylindrical_mono"
      )
...
test_that(
  "Cylinder TMM rejects unsupported general-angle stored scattering requests",
  {
    old_opt <- options(acousticTS.warn_tmm_cylinder = FALSE)
    on.exit(options(old_opt), add = TRUE)
    density_sw <- 1026.8
...
test_that(
  "tmm_diagnostics provides physics-based checks for stored TMM objects",
  {
    old_opt <- options(acousticTS.warn_tmm_cylinder = FALSE)
    on.exit(options(old_opt), add = TRUE)
    density_sw <- 1026.8
    ...
    expect_true(all(c(
      "shape", "coordinate_system", "boundary",
      "frequency"
    ) %in% names(sphere_diag$summary)))
    ...
    expect_equal(min(prolate_diag$continuation$aspect_ratio), 1,
      tolerance =
        1e-12
    )
    expect_equal(
...
test_that(
  paste0(
    "TMM sphere-to-spheroid continuation starts from the exact equal-volume ",
    "sphere"
  ),
  {
    density_sw <- 1026.8
    ...
    diag <- tmm_diagnostics(prolate_obj,
      continuation_steps = 5L, n_theta = 21,
      n_phi = 41
    )
...
test_that("TMM rejects unsupported shapes", {
  ...
  expect_error(
    target_strength(object,
      frequency = 38e3, model = "tmm", boundary =
        "liquid_filled"
    ),
