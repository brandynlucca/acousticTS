...
test_that(
  "TMM bistatic summary and product helpers assemble mocked sub-results",
  {
    object <- fls_generate(
...
test_that(
  "TMM plotting helpers normalize quantities and build slice/grid data",
  {
...
test_that(
  paste0(
    "TMM plotting renderers and dispatcher cover polar, heatmap, and slice ",
    "branches"
  ),
  {
    grid <- list(
    ...
    expect_error(
      acousticTS:::.plot_tmm_scattering_polar(
        modifyList(grid, list(sigma_scat_dB = matrix(NA_real_,
          nrow = 2, ncol =
            2
        )))
      ),
