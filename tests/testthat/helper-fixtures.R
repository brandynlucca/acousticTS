...
fixture_cylinder <- function(boundary_type) {
  ...
  # Shape
  switch(boundary_type,
    fixed_rigid = fls_generate(
      shape = cylinder_shape,
      g_body = NA,
      h_body = NA
    ),
    pressure_release = fls_generate(
      shape = cylinder_shape,
      g_body = NA,
      h_body = NA
    ),
