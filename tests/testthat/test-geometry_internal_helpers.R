...
test_that(
  "canonicalize_shape internal helpers cover validation and fitting branches",
  {
    shape_in <- arbitrary(
    ...
    expect_error(
      canonicalize_shape("not_a_shape"),
      "'shape' must be a Shape object."
    )
    expect_error(
    ...
    local({
      ...
      expect_error(
        ...
        "Source shape must have positive length."
      )
    })
    ...
    local({
      testthat::local_mocked_bindings(
        .shape_radius_profile = function(position_matrix) {
          rep(
            9,
            nrow(position_matrix)
          )
        },
        .package = "acousticTS"
      )
    ...
    sphere_fit <-
      acousticTS:::.canonicalize_shape_fit_profile_l2(source_metrics, "Sphere")
...
test_that(
  paste0(
    "geometry profile helpers cover fallbacks, aliases, and geometry ",
    "reconstruction"
  ),
  {
    row_major_body <- list(
    ...
    expect_error(
      acousticTS:::brake_df(list(rpos = matrix(1:4, nrow = 2), radius = c(
        0.1,
        0.1
      )), -1),
    ...
    expect_true(all(c("x", "y", "z", "zU", "zL") %in%
      rownames(dwba_profile@body$rpos)))
