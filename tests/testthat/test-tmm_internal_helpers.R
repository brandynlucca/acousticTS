...
test_that(
  ...
  ),
  {
    ...
    sphere_params <- fls_generate(
      shape = sphere_shape, g_body = 1, h_body =
        1
    )@shape_parameters
    prolate_params <- fls_generate(
      shape = prolate_spheroid(
        length_body = 0.2, radius_body = 0.01,
        n_segments = 20
      ),
      g_body = 1,
      h_body = 1
    ...
    prolate_short <- fls_generate(
      shape = prolate_spheroid(
        length_body = 0.04, radius_body = 0.01,
        n_segments = 20
      ),
      g_body = 1,
      h_body = 1
    ...
    oblate_params <- fls_generate(
      shape = oblate_spheroid(
        length_body = 0.04, radius_body = 0.05,
        n_segments = 20
      ),
      g_body = 1,
      h_body = 1
    ...
    expect_true(is.na(acousticTS:::.tmm_prolate_nmax_override(
      sphere_params,
      "fixed_rigid"
    )))
    expect_true(is.na(acousticTS:::.tmm_prolate_nmax_override(
    ...
    expect_equal(acousticTS:::.tmm_prolate_nmax_override(
      prolate_params,
      "fixed_rigid"
    ), 30L)
    expect_equal(acousticTS:::.tmm_prolate_nmax_override(
    ...
    expect_true(is.na(acousticTS:::.tmm_cylinder_nmax_floor(
      sphere_params,
      "fixed_rigid"
    )))
    expect_equal(acousticTS:::.tmm_cylinder_nmax_floor(
      cylinder_params,
      "fixed_rigid"
    ), 40L)
    expect_equal(acousticTS:::.tmm_cylinder_nmax_floor(
    ...
    expect_equal(acousticTS:::.tmm_collocation_nodes(
      cylinder_params,
      "fixed_rigid", 3
    ), 128L)
    expect_equal(acousticTS:::.tmm_collocation_nodes(
...
test_that(
  "stored spherical TMM frequency sweep matches single-frequency solves",
  {
    sphere_params <- fls_generate(
      shape = sphere(radius_body = 0.01, n_segments = 20),
      g_body = 1,
...
test_that(
  paste0(
    "internal TMM diagnostics helpers cover validation and continuation ",
    "branches"
  ),
  {
    density_sw <- 1026.8
    ...
    oblate_obj <- target_strength(
      object = fls_generate(
        shape = oblate_spheroid(
          length_body = 0.03, radius_body = 0.05,
          n_segments = 40
        ),
        g_body = 1,
        h_body = 1,
    ...
    expect_error(
      acousticTS:::.tmm_validate_diagnostic_grid(4, 5),
      "single integer >= 5"
    )
    expect_error(
      acousticTS:::.tmm_validate_diagnostic_grid(5, 4),
      "single integer >= 5"
    )
...
test_that(
  paste0(
    "internal TMM scattering and orientation helpers cover remaining ",
    "validation branches"
  ),
  {
    density_sw <- 1026.8
    ...
    sphere_params <- extract(
      sphere(radius_body = 0.01, n_segments = 20),
      "shape_parameters"
    )
    ...
    expect_equal(
      acousticTS:::.tmm_scalar_angle(NULL, pi / 3, "theta_body"),
      pi / 3
    )
    expect_error(
    ...
    expect_error(
      acousticTS:::.tmm_angle_vector(c(-0.1, 0.2),
        lower = 0, name =
          "theta_scatter"
      ),
      "must be >= 0 radians"
    ...
    expect_error(
      acousticTS:::.tmm_angle_vector(c(0.1, pi + 0.1),
        upper = pi, name =
          "theta_scatter"
      ),
      "must be <="
    ...
    expect_error(
      acousticTS:::.tmm_cylindrical_monostatic_f_bs(
        acoustics_row = data.frame(frequency = 38000, k_sw = wavenumber(
          38000,
          1500
        ), n_max = 8L),
    ...
    expect_error(
      acousticTS:::.tmm_validate_orientation_distribution(structure(
        data.frame(
          theta_body = Inf, phi_body = 0, weights = 1, distribution =
            "quadrature"
        ),
        class = c("TMMOrientationDistribution", "data.frame")
      )),
    ...
    expect_error(
      acousticTS:::.tmm_validate_orientation_distribution(structure(
        data.frame(
          theta_body = 0, phi_body = Inf, weights = 1, distribution =
            "quadrature"
        ),
        class = c("TMMOrientationDistribution", "data.frame")
      )),
    ...
    expect_error(
      acousticTS:::.tmm_validate_orientation_distribution(structure(
        data.frame(
          theta_body = 0, phi_body = 0, weights = -1, distribution =
            "quadrature"
        ),
        class = c("TMMOrientationDistribution", "data.frame")
      )),
    ...
    expect_error(
      acousticTS:::.tmm_average_orientation_direct_inputs(
        c(pi / 2), NULL,
        NA_real_
      ),
...
test_that(
  paste0(
    "TMM shared-incidence point and sphere-modal grid paths match scalar ",
    "evaluators"
  ),
  {
    t_store <- list(
