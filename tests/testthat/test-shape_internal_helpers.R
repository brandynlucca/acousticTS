...
test_that(
  paste0(
    "shape resolution and validator helpers cover canonical constructor ",
    "branches"
  ),
  {
    ...
    expect_identical(
      acousticTS:::.resolve_shape(sphere_shape, list()),
      sphere_shape
    )
    expect_s4_class(
    ...
    expect_error(
      acousticTS:::.validate_shape_requirements("sphere", list(
        radius_body =
          NA_real_
      )),
    ...
    expect_error(
      acousticTS:::.validate_shape_requirements("cylinder", list(
        radius_body =
          0.01
      )),
...
test_that(
  "geometry and component helpers cover fallbacks and validation branches",
  {
    shape_matrix <- cbind(
      x = c(0, 1, 2),
      w = c(1, 2, 3),
    ...
    expect_equal(
      acousticTS:::.geometry_axis_values(shape_matrix,
        axis = "w", context =
          "Shape"
      ),
      c(1, 2, 3)
    ...
    expect_equal(
      acousticTS:::.geometry_axis_values(row_major,
        axis = "zU", row_major =
          TRUE
      ),
      c(0.5, 0.6, 0.7)
    ...
    expect_error(
      acousticTS:::.shape_segment_count(1),
      "'position_matrix' must be a matrix"
    )
    expect_equal(acousticTS:::.shape_segment_count(shape_matrix), 2L)
    expect_equal(
      acousticTS:::.shape_segment_count(row_major, row_major = TRUE),
      2L
    )

    expect_equal(acousticTS:::.scatterer_shape_name(
      sphere(radius_body = 0.01),
      "arbitrary"
    ), "Arbitrary")
    expect_match(
    ...
    expect_equal(
      acousticTS:::.extract_shape_component_row(row_major,
        candidates =
          c("radius"), default = c(9, 9, 9)
      ),
      c(9, 9, 9)
    )
    expect_equal(
      acousticTS:::.extract_shape_component_row(matrix(1:6, nrow = 2),
        candidates = "x"
      ),
      c(1, 3, 5)
    )
    ...
    expect_error(
      acousticTS:::.validate_component_rpos(
        matrix(1, nrow = 4, ncol = 1),
        "Body"
      ),
      "Body shape must have at least two points"
    )
    expect_invisible(acousticTS:::.validate_component_rpos(matrix(1:8,
      nrow = 4,
      ncol = 2
    ), "Body"))
  }
...
test_that(
  "scatterer component builders and ESS helpers cover remaining branches",
  {
    sphere_shape <- sphere(radius_body = 0.01, n_segments = 12)
    ...
    prolate_shape <- prolate_spheroid(
      length_body = 0.08, radius_body = 0.01,
      n_segments = 12
    )
    oblate_shape <- oblate_spheroid(
      length_body = 0.04, radius_body = 0.03,
      n_segments = 12
    )
    arbitrary_shape <- arbitrary(
    ...
    diameter_radius <- acousticTS:::.normalize_shape_diameter(list(radius = c(
      1,
      2, 3
    )))
