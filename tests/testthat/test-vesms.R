...
test_that("vesms_initialize respects explicit viscous-layer controls", {
  ...
  expect_equal(
    out_radius@model_parameters$VESMS$parameters$acoustics$m_limit,
    c(4L, 4L, 4L)
  )
  ...
  expect_equal(
    out_thickness@model_parameters$VESMS$parameters$acoustics$m_limit, c(
      2L,
      3L, 4L
    )
  )
...
test_that(
  "vesms_initialize rejects inconsistent geometry and viscous-layer inputs",
  {
    radius_gas <- 1e-3
    radius_shell <- radius_gas + 0.02e-3
    shear_shell <- 0.2e6
...
test_that(
  "VESMS enforces supported object type and layered-property requirements",
  {
...
test_that(
  paste0(
    "VESMS gives consistent end-to-end results for equivalent outer-radius ",
    "inputs"
  ),
  {
    sphere_obj <- make_vesms_reference_object()
    ...
    expect_equal(out_radius@model$VESMS$f_bs, out_thickness@model$VESMS$f_bs,
      tolerance = 1e-12
    )
    expect_equal(out_radius@model$VESMS$TS, out_thickness@model$VESMS$TS,
      tolerance = 1e-12
    )
  }
)
