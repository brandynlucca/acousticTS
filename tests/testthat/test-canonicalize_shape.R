...
test_that(
  "length-volume canonicalization preserves cylinder length and volume",
  {
    shape_in <- arbitrary(
      x_body = c(0, 0.01, 0.02, 0.03, 0.04),
...
test_that(
  paste0(
    "length-volume spheroidal canonicalization errors when the target family ",
    "is incompatible"
  ),
  {
    shape_in <- arbitrary(
...
test_that(
  "canonicalize_shape handles equal-area width-height arbitrary profiles",
  {
    shape_in <- arbitrary(
