...
def write_oblate_msh(path, semiminor_length, semimajor_length, n_u = 40, n_v = 48):
    nodes = []

    # North pole on the +x semiminor tip.
    nodes.append((semiminor_length, 0.0, 0.0))

    # Interior latitude rings.
    for i in range(1, n_u):
        u = math.pi * i / n_u
        x = semiminor_length * math.cos(u)
        r = semimajor_length * math.sin(u)
        for j in range(n_v):
            v = 2 * math.pi * j / n_v
            y = r * math.cos(v)
            z = r * math.sin(v)
    ...
    # South pole on the -x semiminor tip.
...
if __name__ == "__main__":
    out = Path(
        r"c:\Users\Brandyn\Desktop\acousticTS_upd\scratch\oblate_axis_x_pr_c6_a10_40x48.msh"
    )
    n_nodes, n_triangles = write_oblate_msh(
        out,
        semiminor_length = 0.006,
        semimajor_length = 0.01,
        n_u = 40,
        n_v = 48
    )
    print(out)
